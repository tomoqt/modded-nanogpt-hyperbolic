...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, head_dim=64, curvature=1.0,
...
class Hyperparameters:
    ...
    train_seq_len = 48 * 512  # FlexAttention sequence length
    val_seq_len = 4 * 64 * 512  # FlexAttention sequence length for validation
    # optimization
...
adam_params = [dict(params=head_params, lr=0.022), dict(params=embed_params, lr=0.06),
               dict(params=scalar_params, lr=0.004)]
# small adam epsilon by @YouJiacheng. this is an alternate method of fixing the world_size dependence
# discovered by @fernbear.bsky.social https://x.com/hi_tysam/status/1879692937589875094
optimizer1 = torch.optim.Adam(adam_params, betas=(0.8, 0.95), eps=1e-10, fused=True)
optimizer2 = Muon(hidden_matrix_params, lr=0.002, momentum=0.95, rank=rank, world_size=world_size)
